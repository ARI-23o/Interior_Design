...
export const TrustStatsSection: React.FC = () => {
  return (
    <section className="bg-canvas-soft border-b border-border-luxury py-10 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-6 md:gap-8">
          {studioInfo.stats.map((stat, idx) => (
            <div 
              key={idx} 
              className="p-4 sm:p-6 bg-canvas border border-border-luxury text-center sm:text-left transition-all hover:border-bronze group shadow-sm"
            >
        ...
        <div className="pt-6 sm:pt-8 border-t border-border-luxury/70">
          <div className="text-center mb-4 sm:mb-6">
            <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-bronze font-semibold">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 sm:gap-3">
            {studioInfo.materialPartners.map((partner, i) => (
              <div
                key={i}
                className="bg-canvas/80 border border-border-luxury/80 p-2 sm:p-3 text-center transition-all hover:border-bronze hover:bg-canvas"
              >
                <p className="text-[11px] sm:text-xs font-semibold text-charcoal tracking-wide">{partner.name}</p>
                <p className="text-[8.5px] sm:text-[9px] text-charcoal-subtle uppercase tracking-wider mt-0.5">{partner.category}</p>
              </div>
            ))}
          </div>
        </div>
